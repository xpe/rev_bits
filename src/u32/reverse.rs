...
pub fn reverse(input: u32, range: impl RangeBounds<u32>) -> u32 {
...
///     `abcdefghiFEDCBAjklmnopqrstuvwxyz`
/// for min=16, max=23
fn reverse_closed_interval(input: u32, min: u32, max: u32) -> u32 {
...
mod tests {
    ...
    use crate::u32::reverse_u32;

    use super::*;

    #[test]
    fn reverse_compared_to_reference_implementation() {
        fn versus_ref_impl(x: u32, a: u32, b: u32) -> TestResult {
            if a >= BITS || b >= BITS || a >= b {
                return TestResult::discard();
            }
            let y1 = reverse_closed_interval(x, a, b);
            let y2 = reverse_u32(x, a..=b);
            TestResult::from_bool(y1 == y2)
        }
        QuickCheck::new()
            .tests(25_000)
            .max_tests(1_000_000)
            .quickcheck(versus_ref_impl as fn(u32, u32, u32) -> TestResult);
    }

    #[test]
    fn reverse_closed_interval_outside_interval_unchanged() {
        fn outside_interval_unchanged(x: u32, a: u32, b: u32) -> TestResult {
            if a >= BITS || b >= BITS || a >= b {
                return TestResult::discard();
            }
        ...
        QuickCheck::new()
            .tests(25_000)
            .max_tests(1_000_000)
            .quickcheck(outside_interval_unchanged as fn(u32, u32, u32) -> TestResult);
    }

    #[test]
    fn reverse_closed_interval_is_inverse() {
        fn rev_rev(x: u32, a: u32, b: u32) -> TestResult {
            if a >= BITS || b >= BITS || a >= b {
                return TestResult::discard();
            }
        ...
        QuickCheck::new()
            .tests(25_000)
            .max_tests(1_000_000)
            .quickcheck(rev_rev as fn(u32, u32, u32) -> TestResult);
    }
}
